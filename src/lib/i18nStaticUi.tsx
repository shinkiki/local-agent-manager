/**
 * 한국어 리터럴로 그려진 정적 UI를 DOM에서 되짚어 번역하는 장치 — 대응표, 카탈로그
 * 수집, 노드 순회, MutationObserver 치환기가 모두 여기 있다. `i18n.tsx`는 컴포넌트가
 * 부르는 `text(ko, en)` 문맥이라, 둘을 한 파일에 두면 문맥 하나를 고치려 300줄짜리
 * DOM 치환 규칙을 헤집어야 했다. 치환기만 따로 두면 훑기·되짚기·수량 규칙이 한 파일
 * 안에서 끝난다.
 *
...
import { canonicalSourceText, sendableUiCatalogSource, staticEnglishFallback } from "./staticUiText";

const nodeOriginalText = new WeakMap<Node, string>();
const attributeOriginalText = new WeakMap<Element, Map<string, string>>();
/**
 * 컴포넌트가 `text(ko, en)`으로 선언한 영어. 한국어 리터럴로 그려진 노드가 카탈로그에 없을 때
 * 치환기가 빌려 쓴다. 역방향(영어 → 한국어) 표는 두지 않는다 — 그 표는 컴포넌트가 그린 영어를
 * 한국어로 되짚어 카탈로그 영어로 다시 덮는 데만 쓰였고, 그래서 같은 한국어 키의 영어가 두 벌
 * 있을 때 컴포넌트를 고쳐도 화면이 바뀌지 않았다(QA #5·#6). 남은 되짚기 두 갈래(카탈로그
 * 역방향·제3언어 역방향)는 같은 표를 뒤집은 것이라 되짚어 다시 번역해도 원래 값이 나온다.
 */
const registeredUiEnglish = new Map<string, string>();

/** 컴포넌트가 `text(ko, en)`을 부를 때마다 그 짝을 남긴다. 표를 밖으로 내보내지 않는 것은
 *  읽는 쪽이 이 파일 하나뿐이어서다 — 쓰는 쪽만 문을 열어 둔다. */
export function registerUiEnglish(ko: string, en: string): void {
  registeredUiEnglish.set(ko, en);
}
const UI_CATALOG_VERSION = "2026-08-28.2";
const STATIC_UI_SKIP_SELECTOR = "[data-user-content],.markdown-preview,.markdown-source,.doc-editor,.terminal-host,.chat-message > div,.chat-tool pre,.chat-approval pre,.chat-approval-answers strong,.chat-event-error,.message .text-block,.transcript-disclosure pre,.error-banner,.entity-card,.artifact-group,.data-table tbody,.doc-tree,.recent-list,.project-list,code,textarea";
...
const STATIC_UI_EN: Record<string, string> = {
  ...
  "다시 시도": "Retry", "재시도": "Retry", "다시 확인": "Check again", "다시 표시 안 함": "Don't show again", "적용": "Apply", "적용 중…": "Applying…", "저장": "Save", "저장 중…": "Saving…", "취소": "Cancel", "닫기": "Close", "안내 닫기": "Close notice", "삭제": "Delete", "삭제 중…": "Deleting…", "수정": "Edit", "편집": "Edit", "미리보기": "Preview", "전송": "Send", "중단": "Stop", "새 채팅": "New chat", "새 채팅 시작": "Start new chat",
  "선택": "Select", "선택됨": "Selected", "선택 안 함": "None", "활성화": "Enable", "숨기기": "Hide", "숨김 해제": "Unhide", "상세 보기": "Show details", "접기": "Collapse", "펼치기": "Expand", "크게 보기": "Open full view", "원문 보기": "View original", "작업 내역 펼치기": "Expand activity history", "작업 내역 접기": "Collapse activity history",
  ...
  "화면": "Display", "테마": "Theme", "메인 색상": "Accent color", "메시지 표시 방식": "Message display", "마지막 보낸 메시지부터 표시": "Start at your last message", "대화 시작 부분 표시": "Show conversation start", "마지막 대화 표시": "Show latest messages", "자동 (시스템 연동)": "Auto (system)", "라이트 모드": "Light mode", "다크 모드": "Dark mode", "황동": "Brass", "그린": "Green", "블루": "Blue", "시안": "Cyan", "바이올렛": "Violet",
  "운영체제의 라이트/다크 설정을 따라가며, 시스템이 바뀌면 즉시 함께 바뀝니다.": "Follows the operating system and updates immediately when it changes.", "시스템 설정과 관계없이 항상 밝은 화면으로 표시합니다.": "Always use a light appearance regardless of the system setting.", "시스템 설정과 관계없이 항상 어두운 화면으로 표시합니다.": "Always use a dark appearance regardless of the system setting.", "세션을 열면 사용자가 마지막으로 보낸 메시지에서 시작해 응답을 아래로 읽어 내려갑니다.": "Open at your last sent message and read the response downward.", "세션을 열면 대화의 처음부터 보여주고, 새 응답이 도착해도 현재 위치를 유지합니다.": "Open at the start and keep the current position when new responses arrive.", "세션을 열면 가장 최근 대화 위치로 이동하고, 새 응답이 도착하면 최신 메시지를 따라갑니다.": "Open at the latest message and follow new responses.",
  ...
  "언어 및 자동번역": "Language and translation", "UI 언어": "UI language", "공통 저장소": "Shared repository", "라이브러리": "Library", "저장소 경로": "Repository path", "기본 경로": "Default path", "하위 경로": "Managed folders", "현재 장치": "Current device", "사용자 지정": "Custom", "앱 기본값": "App default", "기본 경로로 복귀": "Restore default", "시스템 에이전트": "System agent", "번역 대기": "Translation queued", "번역 중": "Translating", "번역 완료": "Translation complete", "번역 일시 중지": "Translation paused", "일부 번역 실패": "Partially failed", "실패 항목 재시도": "Retry failures", "번역": "Translate", "재번역": "Retranslate", "번역 실패": "Translation failed",
  "설명": "Description", "범위": "Scope", "출처": "Source", "파일": "File", "구성 파일": "Files", "모델": "Model", "최대 턴": "Max turns", "권한 모드": "Permission mode", "도구": "Tools", "시스템 프롬프트": "System prompt", "요약": "Summary", "내용": "Content", "루트": "Root", "크기": "Size", "업데이트": "Updated", "버전": "Version", "본문 잠김": "Content locked", "보관됨": "Archived", "가져옴": "Imported", "위치": "Location", "키": "Key",
...
const STATIC_UI_SOURCES = [
  ...
  "번역 오류", "번역 일시 중지", "번역 중", "번역", "번역 실패", "재번역", "범위", "본문 잠김", "보관됨", "삭제", "상속", "실패", "가져옴", "위치", "키", "공통 저장소", "라이브러리", "저장소 경로", "기본 경로", "하위 경로", "현재 장치", "사용자 지정", "앱 기본값", "기본 경로로 복귀",
...
export function getUiTranslationCatalog(): { version: string; messages: Record<string, string> } {
  ...
  const add = (source: string) => { if (sendableUiCatalogSource(source)) messages[source] = source; };
  for (const source of Object.keys(STATIC_UI_EN)) add(source);
  for (const source of STATIC_UI_SOURCES) add(source);
  for (const source of registeredUiEnglish.keys()) add(source);
  for (const source of collectRenderedUiSources()) add(source);
  return { version: UI_CATALOG_VERSION, messages };
}

/**
 * 요소·텍스트 노드를 한 벌로 훑는다. 문구 수집과 번역 적용은 같은 노드 집합을 봐야 하고,
 * 순회를 따로 두면 한쪽만 손봤을 때 수집된 문구가 번역되지 않는 식으로 어긋난다.
 * TreeWalker는 `root` 자신을 내지 않으므로 대상 자체가 필요한 쪽은 따로 처리한다.
 */
function walkTextAndElements(
  root: Node,
  onText: (node: Node) => void,
  onElement: (element: Element) => void,
): void {
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT);
  let node = walker.nextNode();
  while (node) {
    if (node.nodeType === Node.TEXT_NODE) onText(node);
    else if (node instanceof Element) onElement(node);
...
function skipsStaticUi(node: Node): boolean {
  ...
  return scope === null || scope.closest(STATIC_UI_SKIP_SELECTOR) !== null;
}
...
function collectRenderedUiSources(): string[] {
  ...
  walkTextAndElements(
    root,
    (node) => { if (!skipsStaticUi(node)) collect(node.nodeValue); },
    (element) => {
      if (skipsStaticUi(element)) return;
      for (const attribute of LOCALIZED_ATTRIBUTES) collect(element.getAttribute(attribute));
    },
  );
  return [...sources];
}
...
/**
 * 화면에 보이는 값에 대응하는 한국어 원문. 기억해 둔 원문이 지금 값을 설명하지 못하면
 * — 원문 그대로도, 그 원문의 번역도 아니면 — 앱이 그 자리를 다시 그린 것이므로 옛 원문을
 * 버리고 화면 값에서 되짚는다. 텍스트 노드와 속성은 원문을 담아 두는 곳만 다르고 이
 * 판정은 같아야 한다. 어긋나면 한쪽만 되돌리기가 어긋난 값에 눌러앉는다.
 */
function staticUiSourceText(
  rewrite: StaticUiRewrite,
  current: string,
  remembered: string | undefined,
  remember: (source: string) => void,
): string {
  if (remembered && (current === remembered || current === rewrite.translate(remembered))) return remembered;
  const source = rewrite.restate(current);
  remember(source);
  return source;
}

function translateStaticTextNode(node: Node, rewrite: StaticUiRewrite): void {
  if (node.nodeType !== Node.TEXT_NODE || skipsStaticUi(node)) return;
  const current = node.nodeValue ?? "";
  const next = rewrite.translate(staticUiSourceText(
    rewrite,
    current,
    nodeOriginalText.get(node),
    (source) => nodeOriginalText.set(node, source),
  ));
  if (current !== next) node.nodeValue = next;
}

function translateStaticElement(element: Element, rewrite: StaticUiRewrite): void {
  if (skipsStaticUi(element)) return;
  for (const attribute of LOCALIZED_ATTRIBUTES) {
    const current = element.getAttribute(attribute);
    if (!current) continue;
    const originals = attributeOriginals(element);
    const next = rewrite.translate(staticUiSourceText(
      rewrite,
      current,
      originals.get(attribute),
      (source) => originals.set(attribute, source),
    ));
...
function translateStaticUiTree(target: Node, rewrite: StaticUiRewrite): void {
  if (target.nodeType === Node.TEXT_NODE) { translateStaticTextNode(target, rewrite); return; }
  if (!(target instanceof Element)) return;
  translateStaticElement(target, rewrite);
  walkTextAndElements(
    target,
    (node) => translateStaticTextNode(node, rewrite),
    (element) => translateStaticElement(element, rewrite),
  );
}
...
export function StaticUiLocalization({ locale, messages }: { locale: AppLocale; messages: Record<string, string> }) {
  useEffect(() => {
    ...
    const rewrite: StaticUiRewrite = {
      ...
      restate: (current) => canonicalSourceText(current, messages, STATIC_UI_KOREAN_BY_ENGLISH),
    };
    let applying = false;
...
/** 요소의 속성 원문 보관함. 처음 다루는 요소면 만들어 붙인다. */
function attributeOriginals(element: Element): Map<string, string> {
  let originals = attributeOriginalText.get(element);
  if (!originals) {
    originals = new Map();
    attributeOriginalText.set(element, originals);
  }
  return originals;
}
...
function localizedStaticText(original: string, locale: AppLocale, messages: Record<string, string>): string {
  ...
  if (locale === "en") return withSurroundingWhitespace(original, staticEnglish);
  return withSurroundingWhitespace(original, (core) => messages[core] ?? staticEnglish(core));
}

/**
 * 공백을 걷어낸 문구 하나를 영어로. 등록된 대응이 없으면 문구 전체가 수량 표현일 때만 바꾸고
 * 나머지는 한국어 그대로 둔다 — 부분 치환 규칙은 섞인 문장을 만들었다(QA #51, `staticEnglishFallback`).
 */
function staticEnglish(core: string): string {
  const exact = STATIC_UI_EN[core] ?? registeredUiEnglish.get(core);
  return exact !== undefined ? exact : staticEnglishFallback(core);
}
